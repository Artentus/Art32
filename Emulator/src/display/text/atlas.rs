...
impl FontAtlas {
    pub(super) fn load<R: Read>(reader: R) -> std::io::Result<Self> {
        ...
        let mut ascender = info.metrics.ascender / em_size;
        let mut descender = info.metrics.descender / em_size;

        // We have a top origin in the text renderer
        if y_origin == YOrigin::Bottom {
            // Flip metrics
            ascender = -ascender;
            descender = -descender;
        }
                ...
                let sprite =
                    Option::zip(plane_bounds, atlas_bounds).map(|(plane_bounds, atlas_bounds)| {
                        let mut bounds = Bounds {
                            top: (plane_bounds.top / em_size),
                            bottom: (plane_bounds.bottom / em_size),
